#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""sap-ui5-fiori çevrimdışı test koşucusu (stdlib unittest). SAP'ye, internete ya da npm'e bağlanmaz.

    python tests/run_tests.py            # tümü
    python tests/run_tests.py -k deploy  # ada göre süz
...
def main(argv=None) -> int:
    ...
    print("\n" + "=" * 110)
    print(f"{'SENARYO':62} | {'BEKLENEN':22} | GERÇEKLEŞEN")
    print("-" * 110)
    for ad, beklenen, gercek, ok in H.SONUCLAR:
        print(f"{'OK  ' if ok else 'FAIL'} {ad[:57]:57} | {beklenen[:22]:22} | {gercek}")
