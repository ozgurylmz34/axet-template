...
- Uzun JSON parametresi URL'ye gidiyorsa kısa anahtarlar (`I`/`Q`/`M`/`U`) URL uzunluğunu düşürür — URL sınırı
  sistemden sisteme değişir, DOĞRULANMADI. Ölçülmüş bir vaka ve çözüm: §7.3.

### 7.3 Büyük yük → HTTP 414, ve ikinci modelde `metadataUrlParams` tuzağı

**HTTP 414 (Request-URI Too Long).** V2 `callFunction`'ın `urlParameters`'ı **URL'e** serileşir (gövde ancak
5. argümanla doğar; `undefined` bırakılırsa istek gövdesizdir). Dosya/base64/uzun metin taşıyan bir function
import bu yüzden istek satırını şişirir ve ters proxy duvarına (**SAP Web Dispatcher**, `wdisp/max_permitted_uri_len`)
çarpar — 414 gövdesindeki `This error page was generated by SAP Web Dispatcher` ile kanıtlanır; ICM'in
`icm/HTTP/max_request_size_KB`'si GÖVDE sınırıdır, ilgisizdir. Duvarı yükseltme yoluna gitme: URL'ler access
log'a düşer (dosya içeriği loglanır), yönetilen bulutta parametre müşteride değildir, sınır kalkmaz ötelenir.

**Çözüm — o TEK function import için ikinci model, `manifest.json` DEĞİŞMEZ:**
```javascript
_getBatchModel: function () {
  ...
  return this._oBatchModel;
}
```
`$batch`'te dış URL sabittir (`.../$batch`), yük gövdeye iner (ölçülen tavan ≥ 512 KB, ~60× artış).
⛔ **Denendi, çalışmıyor:** parametreleri `application/x-www-form-urlencoded` gövdeyle göndermek — Gateway
200 döner ama gövdeyi **sessizce yok sayar**; 200 gördün diye geçme, gövdeyi aç. Ham `$batch` gövdesi elle
kurulursa satır sonları **CRLF** ve kapanış ayracından ÖNCE **boş satır zorunludur**, yoksa `400 malformed
syntax` (boyutu işaret etmez).

**`metadataUrlParams` devri seçimlik değil — ASILMA yolunu kapatır.** `ODataModel` iki ayrı havuz kullanır:
CSRF verisi `sServiceUrl` başına paylaşılır, metadata `sMetadataUrl` başına. Manifest'ten kurulan modele UI5
`sap-language` anahtarını **kendisi ekler**; doğrudan `new ODataModel(...)` bunu almaz ⇒ ikinci modelin kendi
`$metadata` GET'i doğar. Asıl bedel bu ek istek değil: metadata yüklemesi **hata alırsa** UI5'in dahili
promise'i hiç settle olmaz ve o modelden yapılan HER istek ona asılır (`.catch`/`timeout` yok) ⇒ istek hiç
gitmez, `success`/`error` hiç koşmaz ⇒ kendi `busy` bayrağını kapatan kod da koşmaz ⇒ **sayfa ömrü boyunca
kalıcı meşgul** (çıkış: F5). Ana modelin `mMetadataUrlParams`'ı devralınırsa ikinci model onun **zaten
çözülmüş** metadata'sını yeniden kullanır ⇒ hem ek istek hem asılma yolu yok olur.
⛔ *"Paylaşılıyor, ek maliyet yok"* hükmü **yalnız kaynak okumasıyla** verilirse yanlış çıkabilir — CSRF
havuzu paylaşılır ama metadata havuzu paylaşılmaz; kanıt AĞ İZİNDEN (ikinci `$metadata` GET'i var mı) gelir.

**Ön yüzde sınır koy:** `FileUploader.maximumFileSize`'ı kodda kullanılan boyut sabitiyle AYNI kaynaktan besle
(iki ayrı literal = sessiz ayrışma) — istek hiç doğmadan reddedilsin. `maximumFileSize` birimi **MB**'dir
(bayt değil — bayt sabitini doğrudan vermek sınırı 1024² kat gevşetir) ve değer **falsy** ise (çözülmeyen binding,
...
edilir (≥ 400 olan var mı). İki mod aynı ekranda karıştırılmaz. **Tek istisna:** dosya/base64/uzun metin taşıyan
function import — `manifest.json` değişmez, o çağrı için ayrı bir `useBatch:true` model açılır (§7.3, FE-46).
