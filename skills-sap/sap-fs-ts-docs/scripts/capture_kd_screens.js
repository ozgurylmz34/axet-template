 ...
 *     "channel": "msedge", "expect_port": 8080,
...
async function main(argv) {
  ...
  if (argv.includes("--dry-run")) {
    console.log("YAPILANDIRMA OK: " + cfg.steps.length + " adım, " + shots + " çekim (tarayıcı açılmadı)");
    return 0;
  }
  ...
  const channel = cfg.channel || process.env.PDF_BROWSER_CHANNEL || "msedge";
  let browser;
  try {
  ...
  try {
    ...
    for (let i = 0; i < cfg.steps.length; i++) {
      ...
      try {
        ...
        if (s.do === "shot") results.push({ label, ok: true });
      } catch (e) {
