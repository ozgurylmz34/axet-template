...
| # | Belirti | Sebep | Çözüm |
|---|---|---|---|
| T1 | Aracı kullanan model `npx playwright-cli install-browser chromium` ve `install-browser firefox` çalıştırdı; yaklaşık **1,06 GB** indirildi (`%LOCALAPPDATA%\ms-playwright`). İndirilen Chromium açılışta çöktü, Firefox hata verdi. **Ölçüldü** | tarayıcı açıkça verilmemişti; etkileşimsiz kipte araç izin sormadı | `kd_ortam.py config` (Chrome'a sabit yapılandırma) + her `open`'da `--browser chrome`; tarayıcı indirmesini reddeden izin kuralı template yapılandırmasındadır. İndirme yapılmışsa silme kararı kullanıcınındır |
