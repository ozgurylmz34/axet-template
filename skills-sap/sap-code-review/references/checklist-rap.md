...
| BE-24 | Released BO'ya `MODIFY ENTITIES … UPDATE/CREATE` yazılmadan önce operasyonun o BO'da açık olduğu doğrulanmamış ("operation is not activated", aktivasyonda çıkar) | Released BDEF'i `adt_get` ile oku; kapalıysa EML değil: released değişiklik-BAPI → yoksa released OData API (sıra `%sap-dev` `write-api-selection.md` ADIM 2 → 3) | BLOCKER | YOK | Ekip dersi; `%sap-odata-backend` outbound-api-call §2.4 |
| BE-08 | Unmanaged / statik action ile yazılan uzun metin kalıcı değil (tampon başarısı ≠ kayıt): `CREATE BY \_Text` tamponda başarılı ama yazılmaz; varsayılan `SAVE_TEXT` de `sy-subrc = 0` dönüp yazmaz | `SAVE_TEXT` çağrısında `savemode_direct = 'X'` var mı; kaydet → geri oku | BLOCKER | YOK | Ekip dersi; `%sap-rap` eml.md §8 |
