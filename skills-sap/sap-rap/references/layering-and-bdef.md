...
| Numara (NR objesi, CHAR key) | Root karakteristiğinde `early numbering` + CCIMP `earlynumbering_create FOR NUMBERING` (`behavior-impl.md` §3). `numbering : managed` yalnız UUID/RAW16 → CHAR key'le kullanma. Numarayı determination ile verme. NR objesini kullanıcı yaratır/yönetir. |
